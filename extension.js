...
function resetProjectContent(target) {
  ...
  fs.writeFileSync(path.join(target, 'panel', 'painel.json'), JSON.stringify({
    ...
    areas: { monitoring: [], ihm: [], panel: [], interface: [], field: [], comunicacao: [] }
  }, null, 2) + '\n');
...
async function openMappingEditor(project) {
  ...
  const areas = ['monitoring', 'ihm', 'panel', 'interface', 'field', 'comunicacao'];
...
const vscode=acquireVsCodeApi(),data=${safeData},config=data.configuration,catalog=data.catalog,areaNames={monitoring:'Monitoramento',ihm:'IHM',panel:'Porta do painel',interface:'Interface cliente',field:'Campo / equipamento',comunicacao:'Comunicação Modbus'},$=id=>document.getElementById(id);let selected=null;
