...
function resetProjectContent(target) {
  ...
  fs.writeFileSync(path.join(target, 'panel', 'painel.json'), JSON.stringify({
    ...
    areas: { monitoring: [], ihm: [], panel: [], interface: [], field: [], simulacao: [], comunicacao: [] }
  }, null, 2) + '\n');
...
async function openMappingEditor(project) {
  ...
  const areas = ['monitoring', 'ihm', 'panel', 'interface', 'field', 'simulacao', 'comunicacao'];
...
const vscode=acquireVsCodeApi(),data=${safeData},config=data.configuration,catalog=data.catalog,areaNames={monitoring:'Monitoramento',ihm:'IHM',panel:'Porta do painel',interface:'Interface cliente',field:'Campo / equipamento',simulacao:'Simulação',comunicacao:'Comunicação Modbus'},$=id=>document.getElementById(id);let selected=null;
...
const collapsed=new Set();function applyFolds(rows){let oculto=null;rows.forEach((item,i)=>{const filho=rows[i+1]&&rows[i+1].depth>item.depth;item.caret.className='routine-caret'+(filho?'':' leaf');item.caret.textContent=filho?(collapsed.has(item.key)?'▸':'▾'):'·';if(oculto!==null&&item.depth>oculto){item.row.classList.add('hidden');return}oculto=null;item.row.classList.remove('hidden');if(filho&&collapsed.has(item.key))oculto=item.depth})}window.addEventListener('message',e=>{const m=e.data;if(m.type==='error'){empty.textContent='Erro: '+m.message;empty.hidden=false;return}if(m.type!=='projects')return;empty.hidden=m.projects.length>0;const alive=new Set(m.projects.map(p=>p.project));for(const [key,c] of cards)if(!alive.has(key)){c.root.remove();cards.delete(key)}for(const p of m.projects){const c=ensure(p),names={run:'RODANDO',paused:'PAUSADO',routine:'PAUSADO',line:'PAUSADO'};c.name.textContent=p.name;c.mode.textContent=names[p.mode]||p.mode;c.mode.className='mode '+p.mode;c.meta.textContent='PID '+p.pid+' · comandos :'+p.port+' · P&ID :'+p.pidPort+' · scan '+p.scan+(p.current?' · próximo: '+p.current.file.split('/').pop()+':'+p.current.line:'');for(const button of c.stepButtons)button.disabled=p.mode==='run';const existing=new Map([...c.routines.children].map(x=>[x.dataset.key,x]));const rows=[];p.routines.forEach((r,i)=>{const key=i+':'+r.file+':'+r.startLine,depth=Number.isInteger(r.depth)?r.depth:0;let row=existing.get(key);if(!row){row=document.createElement('div');row.dataset.key=key;const caret=document.createElement('span');caret.className='routine-caret';const label=document.createElement('span');row.append(caret,label);c.routines.append(row)}else existing.delete(key);const [caret,label]=row.children;row.dataset.depth=depth;row.style.paddingLeft=(3+depth*13)+'px';label.textContent=(r.current?'▶ ':r.executed?'✓ ':'○ ')+(r.title||r.name);row.className='routine-row'+(r.current?' current':r.executed?' executed':'');row.onclick=()=>vscode.postMessage({type:'openRoutine',project:p.project,file:r.file,line:r.current&&p.current?p.current.line:r.startLine});caret.onclick=event=>{event.stopPropagation();if(collapsed.has(key))collapsed.delete(key);else collapsed.add(key);applyFolds(rows)};rows.push({row,caret,key,depth})});for(const row of existing.values())row.remove();applyFolds(rows)}});
</script></body></html>`;
  }
}
